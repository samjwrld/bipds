...
function TestimonialSkeleton() {
  return (
    <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col h-full animate-pulse">
      <div className="flex items-center mb-6">
        <div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-800 mr-4 flex-shrink-0"></div>
    ...
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pt-24 pb-20 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 mb-20">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-display font-bold text-slate-900 dark:text-white mb-6">
            Trusted by <span className="text-red-600 dark:text-red-500">Innovators</span> Across India
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 mb-12">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-display font-bold text-slate-900 dark:text-white">Featured Partnerships</h2>
          ...
          {isLoading ? (
            ...
            testimonials.map((testimonial, idx) => (
              <motion.div
                ...
                className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-md transition-all duration-300 flex flex-col h-full"
              >
                <div className="flex items-center mb-6">
                  <div className="w-12 h-12 rounded-full overflow-hidden mr-4 border-2 border-red-50 dark:border-red-950">
                    <img 
                ...
                <div className="flex mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      className={`w-4 h-4 ${i < testimonial.rating ? 'text-amber-400 fill-amber-400' : 'text-slate-200 dark:text-slate-700'}`} 
                    />
                  ))}
                </div>

                <div className="relative flex-grow">
                  <Quote className="absolute -top-2 -left-2 w-8 h-8 text-red-50 dark:text-red-950/50 opacity-50" />
      ...
      {/* Key Performance Metrics & 98.4% Grant Rate Visualization */}
      <div className="mt-20">
